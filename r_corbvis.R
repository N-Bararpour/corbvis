...
VisoPC<- function(f){
  ...
  sp<- screeplot(p, type = "l", npcs = 10, main = "Screeplot of the first 10 PCs_raw",ylim=c(0,100))
  abline(h = 1, col="red", lty=5)
  legend("topright", legend=c("Eigenvalue = 1"),col=c("red"), lty=5, cex=0.6) ;
  sp.cor<- screeplot(p.c, type = "l", npcs = 10, main = "Screeplot of the first 10 PCs_corrected",ylim=c(0,100))
  abline(h = 1, col="red", lty=5)
  legend("topright", legend=c("Eigenvalue = 1"),col=c("red"), lty=5, cex=0.6) ;
  g<-data.frame(f[1],y);
...
VisoCorr<- function(m){
  ...
  for (i in loop.vector){
    ...
    std<-sd(y[,i])
    par (mfrow=c(3,1))
    myplot<-plot(m[,1],x,col = "black",main=colnames(m)[i+1],xlab="Batch",ylab = "Variable.Abundance")
